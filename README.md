...
| `get_entity(project_id, entity_name)` | Alle Fakten/Relationen zu einer Entität — liefert wie `query` den **Kontext** (`only_need_context`), nicht die vom lokalen Modell formulierte Antwort; sonst läuft es auf geteilter GPU in den MCP-Timeout |
