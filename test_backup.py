"""Selbsttest für backup.py (per-Projekt-Backups).

Seit der Herausloesung aus server.py laeuft das ohne lightrag und mcp:
...
# Env vor dem Import setzen — config.py liest die Pfade beim Modul-Laden.
...
def test_backup_writes_readable_archive():
    name = backup.backup_project("p1")
    path = backup.project_backup_dir("p1") / name
    assert path.exists(), "Archiv nicht geschrieben"
    assert not list(backup.project_backup_dir("p1").glob("*.tmp")), "tmp-Datei nicht aufgeräumt"
    ...
    assert cfg["projects"]["p1"]["signature"] == backup.project_signature("p1")


def test_signature_is_per_project():
    before = backup.project_signature("p1")
    (_tmp / "projects" / "p1" / "neu.json").write_text('{"b": 2}')
    assert backup.project_signature("p1") != before, "neue Datei ändert die Signatur nicht"
    # zweites Projekt ohne Dateien hat eine andere Signatur als p1
    (_tmp / "projects" / "p2").mkdir(exist_ok=True)
    assert backup.project_signature("p2") != backup.project_signature("p1")


def test_rotation_keeps_max_backups():
    bdir = backup.project_backup_dir("p1")
    ...
    backup.backup_project("p1")

    files = backup.list_project_backups("p1")
...
def test_restore_creates_missing_project():
    ...
    name = backup.backup_project("fresh")
    path = backup.project_backup_dir("fresh") / name

    shutil.rmtree(_tmp / "projects" / "fresh")        # Projekt komplett entfernen
    assert not (_tmp / "projects" / "fresh").exists()

    restored = backup.restore_from_archive(path)      # aus dem Archiv wiederanlegen
